    ...
    public void update(){
        for(int i=1; i <= 9;i++){
            switch(i){
                ...
                case 2:{
                    plantsCardList.add(new SunFlower(0, 300, 80 + (i-1)*52, 5,40,80 + 26 + (i-1)*52)); // hỏi tại sao có i
                    break;                     
                }
                case 3:{
    ...
    public void updateShovel(Point point){
        for(Plants i : plantsList){
            if ( i.getXBackyard() == point.getX() && i.getYBackyard() == point.getY()){
                if (i.isImageActive()){
                    if (i.getPlantsValue() % 2 == 0)
                        updateSunValue(i.getPlantsValue()/2, 2);//nếu mà số mặt trời là số chẵn thì sẽ dược nhận 1 nửa số tiền mà cây xẻng đã đào
                    else{
                        updateSunValue((i.getPlantsValue()-5)/2, 2); //nếu mà số mặt trời là số lẻ thì sẽ nhận được (giá trị -5)/2
                    }
                }
                ...
                for(Pea j : peaUpdateList){
                    if (i.getXBackyard() == j.getXBackyard() && i.getYBackyard() == j.getYBackyard()){
                        if (!j.getStop()){
                            j.setShootActive(false);//kết thúc vòng lặp của viên đạn, viên đạn sẽ đứng yên
                            j.setPrepareStop(true); // là cái mạng của cái cây đó chết
                        }
                    }
                }
            }
        ...
        for(Sun i : sunList){
            if (i.getYCoordinate() >= 80)
                if (Math.abs((int) i.getXCoordinate()-e.getPoint().getX()) <= i.getImage().getIconWidth()*1.5 && Math.abs((int) i.getYCoordinate()-e.getPoint().getY()) <= i.getImage().getIconHeight()*1.5){
                    ...
                    i.setYCoordinate(10);
                    sunValue += 25;
                    i.setImageActive(false); //mặt trời đó sẽ mất
