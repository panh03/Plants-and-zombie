...
public class ObjectStable {
    ...
    private int n = 0;//n : số zombie đã chết
    ...
    public void updateTestZombies(int level,int numTurn,int numFinal){
        ...
        this.numTurn = numTurn; //numTurn: số con zombie
        this.numFinal = numFinal; //numFinal: tổng số con lượt cuối
        if (zombiesList.size() + n < numTurn){
            if (delayZombies < 200){
                ...
                else if (zombiesList.size() + n >= 3 && zombiesList.size() + n < list[0]){ //3 : chia vùng của các con dễ và khó trước con số 3 sẽ xuất hiện con khó nhất là mũ cối, sau số 3 sẽ xuất hiện con mũ sắt.
                    if (readyToAddZombies()){
                        if (level == 1)
            ...
            if (checkFinal() && checkFinal == false){ //checkFinal: ktra số con đã chết; checkFinal == false, checkFinal ở đây là kiểm tra coi tới vòng Final chưa
                if (level == 1) // chưa tới Final thì sẽ xuất hiện số case tương ứng
                    addFinalZombies(numFinal,3);
                if (level == 2)
                    addFinalZombies(numFinal, 4);
                if (level == 3)
                    addFinalZombies(numFinal, 5); 
                checkFinal = true; // có nghĩa là đã tới final rồi thì chạy 1 lần thôi.
            }
        }
    }
    ...
    public void updateRemove(){
        ...
        for(Zombies i : zombiesList){
            if (!i.isImageActive()){
                if (i.getZombieExplore()){
                    if  (i.getDeathDelay() <= i.getDeath2()){ // trạng thái lâm sàng 
                        list.add(i); //list.add này là list còn sống, được thêm vào còn sống nếu trạng thái lâm sàng
                    }
                }
                else{
        ...
        n += zombiesList.size() - list.size(); //n: số zombie đã chết, zombie list là số còn sống + đã chết 
